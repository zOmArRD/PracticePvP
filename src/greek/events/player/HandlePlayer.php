...
class HandlePlayer implements Listener
{
    /** @var array */
    ...
    public function pLogin(PlayerLoginEvent $event)
    {
        ...
        AsyncQueue::submitQuery(new SelectQuery("SELECT * FROM settings WHERE ign='{$name}'"), function ($result, $data) {
            ...
            if (sizeof($result) === 0) {
                AsyncQueue::submitQuery(new InsertQuery("INSERT INTO settings(ign, language) VALUES ('{$name}', '{$mode}')"));
            } else {
                NetworkPlayer::$playerData[$name] = $result[0];
    ...
    {
        $player->getLangClass()->applyPlayerLanguage();
    }
